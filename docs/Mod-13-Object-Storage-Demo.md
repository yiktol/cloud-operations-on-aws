...
- Module 13 CloudFormation stack deployed (`Mod-13/cfn-setup.yaml`)
...
The stack creates: an S3 bucket for lifecycle and versioning demos.

```bash
aws cloudformation deploy \
  --template-file Mod-13/cfn-setup.yaml \
  --stack-name mod13-demo \
  --capabilities CAPABILITY_NAMED_IAM
```

### Upload sample files to the bucket
```bash
BUCKET=$(aws cloudformation describe-stacks --stack-name mod13-demo --query "Stacks[0].Outputs[?OutputKey=='BucketName'].OutputValue" --output text)
...
# Delete the stack (removes the bucket)
aws cloudformation delete-stack --stack-name mod13-demo
```
